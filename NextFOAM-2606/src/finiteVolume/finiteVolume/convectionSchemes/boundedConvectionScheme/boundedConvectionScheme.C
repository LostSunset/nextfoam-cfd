...
) const
{
    ...
    if (mesh.foundObject<volScalarField>("massImbalance"))
    {
        const volScalarField& massImbalance =
            mesh.lookupObject<volScalarField>("massImbalance");

        if (massImbalance.dimensions() == faceFlux.dimensions()/dimVolume)
        {
            return
                scheme_().fvmDiv(faceFlux, vf)
              - fvm::Sp(massImbalance, vf);
        }

        WarningInFunction
            << "Found massImbalance but dimensions are inconsistent:" << nl
            << "    massImbalance: " << massImbalance.dimensions() << nl
            << "    expected:      " << faceFlux.dimensions()/dimVolume << nl
            << "Using default bounded convection correction."
            << endl;
    }
