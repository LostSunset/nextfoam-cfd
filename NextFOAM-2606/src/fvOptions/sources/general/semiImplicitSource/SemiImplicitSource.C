...
template<>
void Foam::fv::SemiImplicitSource<Foam::scalar>::addSup
(
    const Foam::volScalarField& rho,
    Foam::fvMatrix<Foam::scalar>& eqn,
    const Foam::label fieldi
)
{
    if (debug)
    {
        Info<< "SemiImplicitSource<scalar>::addSup for source "
            << name_ << endl;
    }
    ...
    // Explicit source
    {
        ...
        tmp<DimensionedField<scalar, volMesh>> tsu;
        ...
        if (iter1.good())
        {
            ...
            const volScalarField* ptr =
                driver.template isResultType<volScalarField>();
            ...
            if (!ptr)
            {
                FatalErrorInFunction
                    << "Expression for Su " << fieldName
                    << " evaluated to <" << driver.resultType()
                    << "> but expected <" << volScalarField::typeName
                    << ">" << endl
                    << exit(FatalError);
            }
            else if (ptr->size() != mesh_.nCells())
            ...
            const scalarField& exprFld = ptr->primitiveField();

            tsu = DimensionedField<scalar, volMesh>::New
            (
                name_ + fieldName + "Su",
                IOobject::NO_REGISTER,
                mesh_,
        ...
        if (tsu.valid())
        {
            if
            (
                (fieldName == "p_rgh" || psi.name() == "p_rgh") 
             && mesh_.foundObject<volScalarField>("massImbalance")
            )
            {
                volScalarField& massImbalance =
                    mesh_.lookupObjectRef<volScalarField>("massImbalance");

                if (massImbalance.dimensions() == tsu().dimensions())
                {
                    massImbalance.primitiveFieldRef() -= tsu().field();
                }
                else
                {
                    WarningInFunction
                        << "Cannot subtract SemiImplicitSource Su from "
                        << "massImbalance due to inconsistent dimensions:" 
                        << nl
                        << "    massImbalance: " << massImbalance.dimensions() 
                        << nl
                        << "    Su:            " << tsu().dimensions() 
                        << nl
                        << endl;
