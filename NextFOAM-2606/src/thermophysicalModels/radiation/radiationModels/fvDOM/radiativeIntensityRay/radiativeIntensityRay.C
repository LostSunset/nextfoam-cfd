    ...
    myRayId_(rayId)
{
    ...
    bool writeIFields(dom_.getOrDefault<bool>("writeIntensityFields", true));
    ...
    forAll(ILambda_, lambdaI)
    {
        ...
        else
        {
            ...
            if (!IDefaultPtr)
            {
                ...
                {
                    IDefaultPtr.reset(new volScalarField(I_));
                }
                else
                {
                    IDefaultPtr.reset
...
void Foam::radiation::radiativeIntensityRay::correct
(
    word& solverName,
    scalar& maxInitialRes,
    scalar& maxFinalRes,
    label& maxIters
)
{
    ...
    forAll(ILambda_, lambdaI)
    {
        ...
        const scalar finalRes =
            ILambdaSol.finalResidual()*omega_/dom_.omegaMax();

        const label nIters = ILambdaSol.nIterations();

        solverName = ILambdaSol.solverName();
        const scalar currentMaxInitialRes = maxInitialRes;
        maxInitialRes = max(initialRes, currentMaxInitialRes);

        if (maxInitialRes != currentMaxInitialRes)
        {
            maxFinalRes = finalRes;
            maxIters = nIters;
        }
    }
}
