...
void Foam::radiation::fvDOM::calculate()
{
    ...
    word solverName("none");
    scalar maxInitialResidual = 0.0;
    scalar maxFinalResidual = 0.0;
    label  maxIters = 0;
    label radIter = 0;
    ...
    {
        radIter++;
        if (Foam::SolverPerformance<scalar>::debug)
        {
            Info<< nl << "fvDOM: Radiation solver for " 
                << IRay_.size()
                << " rays iter:"
                << radIter 
                << endl;
        ...
        maxInitialResidual = 0.0;
        maxFinalResidual = 0.0;
        maxIters = 0;
        ...
        forAll(IRay_, rayI)
        {
            if (!rayIdConv[rayI])
            {
                scalar maxBandInitialResidual = 0.0;
                scalar maxBandFinalResidual = 0.0;
                label maxBandIters = 0;
                IRay_[rayI].correct
                (
                    solverName,
                    maxBandInitialResidual,
                    maxBandFinalResidual,
                    maxBandIters
                );

                maxInitialResidual =
                    max(maxBandInitialResidual, maxInitialResidual);
                maxFinalResidual = max(maxBandFinalResidual, maxFinalResidual);
                maxIters = max(maxBandIters, maxIters);

                if (maxBandInitialResidual < tolerance_)
                {
                    rayIdConv[rayI] = true;
                }
            }
    ...
    } while (maxInitialResidual > tolerance_ && radIter < maxIter_);

    if (Foam::solverPerformance::debug)
    {
        Info << solverName
             << ":  Solving for I"
             << ", Initial residual = "
