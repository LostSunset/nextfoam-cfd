...
//        pRight/(rhoRight*(gammaRight - 1.0)) + 0.5*magSqr(URight) + kRight; //cwo
    ...
    // Step 6: compute eigenvalues
    // derived from algebra by hand, only for Euler equation usefull
    scalar lambda1 = mag(contrVTilde - cTilde - contrVg);
    scalar lambda2 = mag(contrVTilde - contrVg);
    scalar lambda3 = mag(contrVTilde + cTilde - contrVg);
    ...
    const scalar flux1 =
        (
            fluxLeft11 + fluxRight11 - (diffF11 + diffF21 + diffF31)
          - (rhoLeft + rhoRight)*contrVg
        )*0.5;

    const vector flux24 =
        (
            fluxLeft124 + fluxRight124 - (diffF124 + diffF224 + diffF324)
          - (rhoLeft*ULeft + rhoRight*URight)*contrVg
        )*0.5;

    const scalar flux5 =
        (
            fluxLeft15 + fluxRight15 - (diffF15 + diffF25 + diffF35)
          - (rhoLeft*E_L + rhoRight*E_R)*contrVg
        )*0.5;
...
void roeFlux::update(const surfaceScalarField& faceContrVg, bool moving)
{
    if (moving)
    {
