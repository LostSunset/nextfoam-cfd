...
)
{
    ...
    (
        vf.mesh(),
        vf.mesh().ddtScheme("ddt(" + rhoName + ',' + vf.name() + ')')
    ).ref().meshPhi(vf);
}
...
)
{
    if (phi.mesh().moving())
    {
        phi -= rhof*fvc::meshPhi(rhoName, U);
    }
}
...
)
{
    if (tphi().mesh().moving())
    {
        return tphi - rhof*fvc::meshPhi(rhoName, U);
    }
    else
    {
        return tmp<surfaceScalarField>(tphi, true);
...
void Foam::fvc::correctUfRelativePhi
(
    autoPtr<surfaceVectorField>& Uf,
    const surfaceScalarField& phi,
    const volVectorField& U
...
void Foam::fvc::correctRhoUfRelativePhi
(
    autoPtr<surfaceVectorField>& rhoUf,
    const surfaceScalarField& phi,
    const surfaceScalarField& rhof,
...
)
{
    ...
    if (mesh.dynamic())
    {
        rhoUf() = rhof*fvc::interpolate(U);
        surfaceVectorField n(mesh.Sf()/mesh.magSf());
        rhoUf() += 
            n*
            (
                fvc::absolute(phi, rhof, rhoName, U)/mesh.magSf() 
              - (n & rhoUf())
            );
    }
}
