...
void spectralRadius::calcSpectralRadii
(
...
) const
{
    ...
    spectralRadiiInv = w_*(mag(contrVavg - contrVg) + speedOfSoundAvg)*magSf;
    spectralRadiiVis = sqr(magSf)*max(4./3., gammaAvg)*alphaEffAvg/rhoAvg;

    spectralRadiiown =
        w_*(mag(contrVLeft - contrVg) + speedOfSoundLeft)*magSf
      + magSf*max(4./3., gammaLeft)*alphaEffLeft/rhoLeft/mag(dLoc);

    spectralRadiinei =
        w_*(mag(contrVRight - contrVg) + speedOfSoundRight)*magSf
      + magSf*max(4./3., gammaRight)*alphaEffRight/rhoRight/mag(dLoc);
}
...
void spectralRadius::evaluateSpectralRadii()
{
    ...
    forAll(owner, faceI)
    {
        ...
        spectralRadius::calcSpectralRadii
        (
    ...
    forAll(U_.boundaryField(), patchi)
    {
        ...
        const fvPatchScalarField& palphaEff = alphaEff.boundaryField()[patchi];
...
void spectralRadius::evaluateSpectralRadii
(
    const surfaceScalarField& faceContrVg
)
{
    // Get face-to-cell addressing: face area point from owner to neighbour
    const labelUList& owner = mesh_.owner();
    ...
    forAll(owner, faceI)
    {
        ...
        (
            ...
            Sf[faceI],          // face vector
            magSf[faceI],       // face area
            faceContrVg[faceI]  // face velocity
        ); 
    }
    ...
    forAll(U_.boundaryField(), patchi)
    {
        ...
        const fvsPatchVectorField& pSf = Sf.boundaryField()[patchi];
        const fvsPatchScalarField& pMagSf = magSf.boundaryField()[patchi];
        const fvsPatchScalarField& pVg = faceContrVg.boundaryField()[patchi];
...
{
    evaluateSpectralRadii();
}


void spectralRadius::update(const surfaceScalarField& faceContrVg, bool moving)
{
    if (moving)
    {
        evaluateSpectralRadii(faceContrVg);
    }
    else
    {
        evaluateSpectralRadii();
