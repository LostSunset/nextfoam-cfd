...
//    const scalar aRight = Foam::sqrt(gammaRight*pRight/rhoRight);
    ...
    const scalar SRight = max(qRight + aRight, contrUTilde + aTilde);
...
//    const scalar aRight = Foam::sqrt(gammaRight*pRight/rhoRight);
    ...
    const scalar contrURelTilde = (UTilde & normalVector) - contrVg;
    ...
    rhoFlux = convectionSpeed*rhoState*magSf;
    rhoUFlux = (convectionSpeed*rhoUState + pState*normalVector)*magSf;
    rhoEFlux = (convectionSpeed*(rhoEState + pState) + pState*contrVg)*magSf;
}
...
void hllcFlux::update(const surfaceScalarField& faceContrVg, bool moving)
{
    if (moving)
    {
