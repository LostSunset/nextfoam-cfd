...
:
    ...
    C1_(dict.getOrDefault<scalar>("C1", 0.0)),
    C2_(dict.getOrDefault<scalar>("C2", 1.0)),
    ppMin_((boundBox(pp.localPoints())).min()),
    time_(time),
    patch_(pp),
...
tmp<vectorField> atmBoundaryLayer::flowDir(const fvPatch& fvp) const
{
    const scalar t = time_.timeOutputValue();
    const vector dir(flowDir_->value(t));
    const scalar magDir = mag(dir);
...
tmp<vectorField> atmBoundaryLayer::U(const fvPatch& fvp) const
{
